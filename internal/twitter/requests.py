...
def get_tweets_for_user(handle):
    ...
    url = "https://api.twitter.com/2/tweets/search/recent?max_results=100&{}&tweet.fields=public_metrics".format(q)
    return url
...
def get_tweets_for_usr_maxid(handle, until_id):
    ...
    url = "https://api.twitter.com/2/tweets/search/recent?max_results=100&{}&{}&tweet.fields=public_metrics".format(
        q, until
    )
    return url
